...
assert(process.env.MAINNET_RPC_URL)
...
//   network.provider.send('hardhat_setBalance', [address, dirtyFix(balance._hex)])
// }
...
export async function setTokenBalance(
  ...
  const slot = dirtyFix(keccak256(encodeSlot(['address', 'uint'], [account.address, index])))
...
export function encodeSlot(types: string[], values: any[]) {
